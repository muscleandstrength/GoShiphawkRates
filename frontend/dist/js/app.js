document.addEventListener('DOMContentLoaded', function () {
  ...
  // Fetch carriers on page load
  fetch('/api/carriers')
    .then(response => response.json())
    .then(carriers => {
      const carrierFilter = document.getElementById('carrierFilter')
      carrierFilter.innerHTML = '<option value="" selected>All Carriers</option>'
  ...
  function getQuotes (event) {
    ...
    const selectedCarriers = Array.from(carrierFilterSelect.selectedOptions)
      .map(option => option.value)
      .filter(value => value !== '') // Filter out the "All Carriers" option
    ...
    const requestData = {
      ...
      carrier_filter: selectedCarriers
    }
    ...
    if (hasRequiredAddressFields(destinationAddress)) {
      requestData.destination_address =
        cleanEmptyAddressFields(destinationAddress)
    }
  ...
  function displayRates (rates) {
    ...
    rates.forEach(rate => {
      ...
      clone.querySelector('.carrier-name').textContent =
        rate.carrier || 'Unknown Carrier'
      clone.querySelector('.service-name').textContent =
